...
class NodalLoadItem(QGraphicsItem):
    def __init__(self, node_tag: int, values: tuple[float, ...]) -> None:
        super().__init__()
        ...
        self.setToolTip(
            f"Node {node_tag} | Fx={self.fx:g} kN | Fy={self.fy:g} kN | "
            f"Mz={self.mz:g} kN*m"
        )
    ...
    def _draw_moment(self, painter: QPainter) -> None:
        ...
        arrow_head.closeSubpath()
        painter.drawPath(arrow_head)
        self._draw_label(painter, f"Mz {self.mz:g} kN*m", QPointF(28.0, 5.0))
